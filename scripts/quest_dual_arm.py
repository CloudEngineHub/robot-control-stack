...
class QuestReader(threading.Thread):
    ...
    def __init__(self, env: RelativeActionSpace):
        ...
        self.controller_names = ["l", "r"]
        self._trg_btn = {"l": "LTr", "r": "RTr"}
        self._grp_btn = {"l": "LG", "r": "RG"}
        self._clb_btn = {"l": "X", "r": "A"}
        self._home_btn = "B"
        ...
        while not self._exit_requested:
            ...
            for controller in self.controller_names:
                last_controller_pose = Pose(
                    pose_matrix=np.array(data["pos"][controller]),
                )

                if data["buttons"][self._clb_btn[controller]] and (
                    self._prev_data is None or not self._prev_data["buttons"][self._clb_btn[controller]]
                ):
                    print("clb button pressed")
                    with self._resource_lock:
                        self._set_frame[controller] = last_controller_pose

                if data["buttons"][self._trg_btn[controller]] and (
                    self._prev_data is None or not self._prev_data["buttons"][self._trg_btn[controller]]
                ):
                ...
                elif not data["buttons"][self._trg_btn[controller]] and (
                    self._prev_data is None or self._prev_data["buttons"][self._trg_btn[controller]]
                ):
                    # released
                    transform = Pose(
                        translation=(
                ...
                elif data["buttons"][self._trg_btn[controller]]:
                    # button is pressed
                    with self._resource_lock:
                        self._last_controller_pose[controller] = last_controller_pose
                else:
                ...
                if data["buttons"][self._grp_btn[controller]] and (
                    self._prev_data is None or not self._prev_data["buttons"][self._grp_btn[controller]]
                ):
                    # just pressed
                    self._grp_pos[controller] = 0
                if not data["buttons"][self._grp_btn[controller]] and (
                    self._prev_data is None or self._prev_data["buttons"][self._grp_btn[controller]]
                ):
                    # just released
